...
from app.clients.auth_client import AuthClient  # Add this import
...
async def get_current_user(jwt_token: str) -> str:
    """
    Get current user information from the auth service
    ...
    """
    try:
        # Use AuthClient instead of make_authenticated_request
        auth_client = AuthClient()
        user_data = await auth_client.get_current_user(jwt_token)
        
        if user_data:
            # Only return non-sensitive information
            name = user_data.get('name', 'Unknown')
            email = user_data.get('email', 'Unknown')
            return f"You are {name} (Email: {email})."
        else:
            return "I am unable to retrieve your user information at this time due to an unknown error. Please try again later."
            
    except Exception as e:
        return f"I am unable to retrieve your user information at this time due to an unknown error. Please try again later."

async def get_user_tasks(jwt_token: str) -> str:
    """
    Get tasks assigned to the current user from the task service
    ...
    """
    try:
        # Use TaskClient to get user tasks
        from app.clients.task_client import TaskClient
        task_client = TaskClient()
        
        # Make the API call using the existing TaskClient method
        url = f"{task_client.base_url}/api/tasks/user/assigned"
            ...
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    if isinstance(tasks_data, list):
                        ...
                        return f"You have {len(tasks_data)} task(s) assigned to you:\n" + "\n".join(tasks_info)
                    else:
                        # Handle single task (fallback)
                        task_data = tasks_data
                        return f"You have 1 task: {task_data.get('title', 'Unknown')} - Status: {task_data.get('status', 'Unknown')} - Priority: {task_data.get('priority', 'Unknown')} - Deadline: {task_data.get('deadline', 'Unknown')}"
                else:
                    return f"I am sorry, I was unable to retrieve your tasks. However, you can view and manage your tasks on the Kanban board at {FRONTEND_URL}/kanban. The Kanban board provides a visual task management system with drag-and-drop functionality. You can also schedule your tasks and events on the Calendar at {FRONTEND_URL}/calendar, which is integrated with the Kanban board."
...
def create_agent_tools(jwt_token: str):
    ...
    tools = [
        StructuredTool.from_function(
            ...
            description="Get current user information from the auth service. Use this when the user asks about their profile, account, personal information, or asks 'who am I?'",
