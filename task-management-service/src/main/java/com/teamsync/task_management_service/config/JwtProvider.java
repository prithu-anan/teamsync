...
public class JwtProvider {
    ...
    public boolean validateToken(String token) {
        try {
            ...
            log.debug("JWT token starts with: {}", token.substring(0, Math.min(20, token.length())));
    ...
    public String generateServiceToken() {
        log.debug("Generating service token for task-management-service");
        log.debug("Secret key length: {}", key.getEncoded().length);
        log.debug("Expiration time: {} ms", jwtConstant.getExpirationTime());
        
        String token = Jwts.builder()
                .setSubject("task-management-service")
