...
REM Test public test endpoint (should work without authentication)
echo 2. Testing public test endpoint:
curl -s http://localhost:8080/test/public
echo.

REM Test secure endpoint without token (should fail)
echo 3. Testing secure endpoint without token (should fail):
curl -s -w "HTTP Status: %%{http_code}" http://localhost:8080/test/secure
echo.

REM Test secure endpoint with invalid token (should fail)
echo 4. Testing secure endpoint with invalid token (should fail):
curl -s -w "HTTP Status: %%{http_code}" -H "Authorization: Bearer invalid-token" http://localhost:8080/test/secure
echo.
